...
def restore_redacted_settings_for_update(
    client_settings: dict, current_settings: dict
) -> dict:
    """Replace redacted placeholders in client updates with current secret values."""
    restored = copy.deepcopy(client_settings)
    for path in _SECRET_SETTING_PATHS:
        if _get_nested_value(restored, path) == _REDACTED_SECRET_VALUE:
            _set_nested_value(
                restored,
                path,
                _get_nested_value(current_settings, path) or "",
            )
    return restored
...
def _strip_env_secrets_for_persistence(settings_dict: dict) -> dict:
    ...
    Runtime settings may need concrete values, but persisted config should not
    ...
    for env_name, path in _ENV_SECRET_SETTING_PATHS:
        ...
        current_value = _get_nested_value(sanitized, path)
        if current_value == _REDACTED_SECRET_VALUE:
            _set_nested_value(sanitized, path, _get_nested_value(persisted, path) or "")
            continue
        if env_value and current_value == env_value:
            _set_nested_value(sanitized, path, _get_nested_value(persisted, path) or "")
    return sanitized
